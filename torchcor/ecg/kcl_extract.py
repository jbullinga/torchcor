...
# ── Tag sets (CARP file numbering) ─────────────────────────────────────────────
ICD_TAGS   = set(range(26, 34))          # 26-33 = hardware
VENT_TAGS  = {24, 25, 34, 35}           # biventricular region
BLOOD_TAGS = {24, 25}                    # blood pools (iso, no Vm source)
MYO_TAGS   = {34, 35}                    # myocardium (bidomain source)
SKIN_TAG   = 2                           # skin surface
BONE_TAG   = 3                           # skeleton (for electrode placement)
...
def extract(
    ...
    # LV endocardium: nodes shared by tag 35 (LV wall) AND tag 25 (LV blood)
    lv_endo_torso = find_interface_nodes(t_elems, t_regions, tag_a=35, tag_b=25)
    ...
    # RV endocardium: nodes shared by tag 34 (RV wall) AND tag 24 (RV blood)
    rv_endo_torso = find_interface_nodes(t_elems, t_regions, tag_a=34, tag_b=24)
