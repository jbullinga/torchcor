...
def launch(vtu_path: Path) -> None:
    ...
    # ── DICOM coordinate transform ────────────────────────────────────────────
    # 180° rotation around Z-axis: flip both X and Y simultaneously.
    # This keeps anatomy correct (no mirror) and brings the anterior face to +Y
    # so the default camera (at max-Y) shows the front.
    # Result: +x = patient left (DICOM), +y = back→front, +z = inferior→superior.
    pts = mesh.points.copy().astype(float)
    pts[:, 0]  = -pts[:, 0]                      # } 180° Z-rotation:
    pts[:, 1]  = -pts[:, 1]                      # }   negate both X and Y
    pts[:, 1] -= float(pts[:, 1].min())          # shift Y: posterior=0, anterior=max
    pts[:, 2] -= float(pts[:, 2].min())          # shift Z: inferior=0, superior=max
    mesh.points = pts

    # ── Build per-tissue surface meshes (with disk cache) ────────────────────
    # Surface extraction is slow (~1–3 min for 2 M-element mesh); cache to disk.
    cache_path = vtu_path.with_suffix(".surfaces_v4.pkl")  # v4: 180° Z-rotation + single ECG row
    ...
    if cache_path.exists():
        ...
        for i, tag in enumerate(tags, 1):
            ...
            tname = style.get("name", f"Tag {tag}")
    ...
    elec_pos = _place_ecg_electrodes(
        surfaces.get(3),    # bones (ribs, sternum)
        surfaces.get(2),    # skin
        mesh.points,
        lv_surf=surfaces.get(27),  # LV wall — anchors 5th ICS Z level
    )
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        vtu_path = _find_vtu()
        if vtu_path is None:
            print("No VTU file found. Pass path explicitly:")
